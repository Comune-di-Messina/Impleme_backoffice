...
export class AuthService {
  ...
  constructor(
    ...
    //     console.error(event, 'OAuthErrorEvent')
    //   } else {
    //     console.log(event, 'DEBUG: oauthService event')
    //   }
    // })
    ...
    // this.oauthService.events.pipe(filter(e => ['logout'].includes(e.type))).subscribe(() => {
    //   this.oauthService.logOut()
    // })

    this.oauthService.events.pipe(filter(e => ['token_received'].includes(e.type))).subscribe(e => {
      this.store.dispatch(new UserAction.TokenReceived())
    })
    ...
    this.oauthService.events
      ...
      .subscribe(e => {
        ...
        this.store.dispatch(
          new UserAction.LoadCurrentAccountSuccessful({
            login: claims.sub as string,
            email: claims.email as string,
            role: claims.groups.split(',') as string[],
          }),
        )
      })
    ...
    this.oauthService.events
      ...
      .subscribe(e => {
        this.store.dispatch(new UserAction.FlushUser())
      })
      ...
      this.oauthService
        ...
        .then(() => {
          ...
          if (
            ...
            console.log(`There was state, so we are sending you to: of ${this.oauthService.state}`)
  ...
  private revokeToken(token: string) {
    ...
    this.http.post(revocationUrl, urlSearchParams.toString(), { headers }).subscribe(
      result => {
        console.log(
