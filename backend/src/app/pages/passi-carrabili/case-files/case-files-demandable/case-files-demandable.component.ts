...
import { PraticheResponse } from './case-files-demandable-resolver.service'

@Component({
  selector: 'app-case-files-demandable',
  templateUrl: './case-files-demandable.component.html',
  styleUrls: ['./case-files-demandable.component.scss'],
  providers: [ReservationIdPipe],
})
export class CaseFilesDemandableComponent implements OnInit {
  ...
  get availablePratiche(): Pratica[] {
    return this.pratiche.filter((pratica, i) => {
      const itemDemandable = this.praticheDemandable.find(
  ...
  claimTask(pratica: Pratica, cb: () => void) {
    const task = this.praticheDemandable.find(
    ...
    this.managePraticheService.assignTask(task.key).subscribe(data => {
      this.msg.success('Pratica presa in carico con successo')
      cb()
    })
  }
